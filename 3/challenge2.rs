...
fn steps_to_reach_point(p: &Point, points: &Vec<Point>) -> i64 {
    let mut steps = 0;
...
fn main() {
    ...
    let points1v = get_points_as_vec(&descriptions.0);
    let points2v = get_points_as_vec(&descriptions.1);

    let intersect = points1.intersection(&points2);
    let sol = intersect.into_iter().map(|p| steps_to_reach_point(p, &points1v) + steps_to_reach_point(p, &points2v)).min();
